...
# PRESET FFmpeg (QP 32 + Audio en modo Copia + Subs en modo copia)
FFMPEG_ARGS="-map 0 -c:v hevc_nvenc -preset slow -rc vbr -cq 36 -b:v 0 -spatial_aq 1 -c:a copy -c:s copy"

# Nombre para el etiquetado de video (el audio será dinámico)
NEW_V_CODEC="HEVC"
...
get_info() {
    ...
    # Limpieza básica para evitar nombres extraños si el audio es desconocido
    if [ -z "$OLD_V_CODEC" ]; then OLD_V_CODEC="unk"; fi
    if [ -z "$OLD_A_CODEC" ]; then OLD_A_CODEC="unk"; fi
    
    # Convertimos el nombre del codec de audio original a mayúsculas para que la etiqueta quede bonita (ej: ac3 -> AC3)
    OLD_A_CODEC=$(echo "$OLD_A_CODEC" | tr '[:lower:]' '[:upper:]')
}

echo "--- INICIANDO PROCESO (MODO ESPEJO CON ETIQUETAS Y AUDIO INTACTO) ---"
...
find . -type f \( -name "*.mkv" -o -name "*.mp4" -o -name "*.avi" \) -not -path "./\[byte\]*" | while read -r FILE; do
    ...
    # Formato: [byte] [CODEC_V - CODEC_A_ORIGINAL] [SUBS] NombreOriginal.ext
    TAG_INFO="[${NEW_V_CODEC} - ${OLD_A_CODEC}]"
    NEW_FILENAME="[byte] ${TAG_INFO} ${NAME_NO_EXT}${SUBS_STRING}.byte.${EXTENSION}"
